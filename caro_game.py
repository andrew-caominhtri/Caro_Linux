...
# --- HÀM LẤY ĐƯỜNG DẪN TÀI NGUYÊN CHO PYINSTALLER ---
def resource_path(relative_path):
    """Lấy đường dẫn tuyệt đối đến tài nguyên, hoạt động cho cả dev và PyInstaller."""
    try:
        # PyInstaller tạo ra thư mục tạm _MEIPASS khi ứng dụng chạy từ file .exe
...
class CaroGame:
    ...
    def __init__(self, root):
        ...
        self.root.configure(
            bg="#1e1e2e"
        )  # Màu nền tối hiện đại (Cyberpunk style)
        ...
        self.btn_pvp = tk.Button(
            self.menu_frame,
            text="Người vs Người",
            font=("Arial", 10, "bold"),
            command=lambda: self.set_mode("PvP"),
            bg="#89b4fa",
            fg="#11111b",
            relief=tk.FLAT,
            bd=0,
        ...
        self.btn_pve = tk.Button(
            self.menu_frame,
            text="Chơi với Máy",
            font=("Arial", 10, "bold"),
            command=lambda: self.set_mode("PvE"),
            bg="#45475a",
            fg="#cdd6f4",
            relief=tk.FLAT,
            bd=0,
    ...
    def player_move(self, r, c):
        ...
        if self.check_win(r, c, self.current_player):
            messagebox.showinfo(
                "Victory!", f"Người chơi {self.current_player} đã chiến thắng!"
            )
            self.game_over = True
            return
    ...
    def make_move(self, r, c, player):
        ...
        color = (
            "#89dceb" if player == "X" else "#f38ba8"
        )  # X xanh neon nhạt, O đỏ hồng
    ...
    def bot_move(self):
        ...
        if self.check_win(best_move[0], best_move[1], "O"):
            messagebox.showinfo(
                "Defeat!", "Máy đã chiến thắng! Hãy thử lại nhé."
            )
            return True
        return False
    ...
    def reset_game(self):
        ...
        for r in range(self.size):
            for c in range(self.size):
                self.buttons[r][c].config(
                    text="", state=tk.NORMAL, bg="#181825"
                )
...
if __name__ == "__main__":
    ...
    # --- NẠP ICON DÙNG HÀM RESOURCE_PATH ---
    icon_png = resource_path("images.png")
    icon_ico = resource_path("images.ico")

    try:
        # Sử dụng icon photo từ file PNG (True giúp áp dụng cho cả Taskbar và Window Title bar)
        app_icon = tk.PhotoImage(file=icon_png)
        root.iconphoto(True, app_icon)
    except Exception:
        try:
            # Dự phòng cho Windows nếu dùng file .ico
